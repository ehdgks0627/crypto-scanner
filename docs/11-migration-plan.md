...
구현에서는 후보 매핑을 `system/backend/migration_engine/mapping_rules.json` 설정 파일로 분리한다. 엔진은 이 파일의 `rules`를 순서대로 평가해 첫 번째 매칭 규칙을 적용하며, 규칙에는 `match_regex`, `asset_types`, `kind`, `hybrid_set`, `replace_set`, `classically_weak_if_contains` 등이 포함된다. 따라서 RSA 인증서와 RSA key/protocol처럼 같은 알고리즘 계열이라도 자산 용도에 따라 서명 전환과 KEM 전환을 다르게 권고할 수 있다.
