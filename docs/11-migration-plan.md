...
strategy_recommendation(asset):
    if asset.lifespan_years >= 10:
        ...
        primary = "hybrid"
    elif algorithm.classical_strength_bits < 100:
...
**클라이언트 측 생성을 기본으로 한다.** 별도 API를 추가하지 않고, 이미 받은 `migration-plan` 응답을 가공하여 Markdown으로 직렬화 후 Blob 다운로드.
