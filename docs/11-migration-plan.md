...
| PDF 보고서 | ✗ | ✓ |
| 우선순위 자동 스케줄링 | ✗ | ✓ |
| Crypto Agility 점수/장애물/Playbook | ✓ | (확장) |

## 11.2 권고 알고리즘 결정 룰

### 11.2.1 자산 종류별 후보 매핑

구현에서는 후보 매핑을 `system/backend/migration_engine/mapping_rules.json` 설정 파일로 분리한다. 엔진은 이 파일의 `rules`를 순서대로 평가해 첫 번째 매칭 규칙을 적용하며, 규칙에는 `match_regex`, `asset_types`, `kind`, `purpose`, `hybrid_set`, `replace_set`, `classically_weak_if_contains` 등이 포함된다. 따라서 RSA 인증서와 RSA key/protocol처럼 같은 알고리즘 계열이라도 자산 용도에 따라 서명 전환과 KEM 전환을 다르게 권고할 수 있다. API 응답은 `asset_purpose`로 키 교환, 디지털 서명, 키 합의, 장기 서명 등 권고 목적을 함께 제공한다.

| 현재 알고리즘 | 1차 후보 (Hybrid) | 2차 후보 (Replace) |
|---|---|---|
| RSA-1024/2048/3072/4096 (서명) | RSA-2048 + ML-DSA-65 (hybrid cert) | ML-DSA-65 |
| ECDSA P-256 / prime256v1 / secp256r1 (서명) | ECDSA P-256 + ML-DSA-65 (hybrid cert) | ML-DSA-65 |
| ECDSA P-384/521 (서명) | ECDSA P-384 + ML-DSA-87 (hybrid cert) | ML-DSA-87 |
| Ed25519 (서명) | Ed25519 + ML-DSA-65 | ML-DSA-65 |
| 장기 보존용 서명 (archive/audit/timestamp 등) | 기존 서명 + SLH-DSA-SHA2-128s | SLH-DSA-SHA2-128s |
| RSA (키 교환: key/key_agreement/protocol) | X25519 + ML-KEM-768 (hybrid KEM) | ML-KEM-768 |
| DH (modp14/modp15) | X25519 + ML-KEM-768 | ML-KEM-768 |
| ECDH P-256 / ECP-256 | X25519 + ML-KEM-768 | ML-KEM-768 |
| ECDH P-384/521 / X448 | X448 + ML-KEM-1024 | ML-KEM-1024 |
| X25519 / Curve25519 / DH group14/modp2048 | X25519 + ML-KEM-768 | ML-KEM-768 |
...
> 본 매핑은 현 시점(2026-04 기준)의 NIST 표준화 결과(FIPS 203/204/205)와 IETF의 hybrid 권고를 바탕으로 한다. 후보 알고리즘은 설정 파일로 관리되며 향후 표준 변동 시 코드 수정 없이 규칙 파일을 갱신하는 방식으로 반영한다.
...
class MigrationRecommendation:
    ...
    playbook: list[MigrationPlaybookStep]
...
class Recommendation:
    ...
    target_algorithm_set: list[str]
    final_algorithm_set: list[str]
    phase: str                 # "hybrid_first" | "replace_now" | "monitor"
    blockers: list[str]        # runtime_capability_unknown 등
    rollback: str
    validation: list[str]
    rationale: str
...
@dataclass
class MigrationPlaybookStep:
    order: int
    kind: str                  # remove_blockers | enable_hybrid | replace_algorithm | monitor
    title: str
    action: str
    validation: str
```

### 11.4.2 Crypto Agility 평가 원칙

마이그레이션 권고는 단순 알고리즘 치환이 아니라, 운영 중 알고리즘을 교체할 수 있는 능력까지 함께 평가한다.

| 항목 | 예시 |
|---|---|
| Enabler | `config_policy`, `automated_rotation`, `rollback_supported`, `rescan_validation`, `owner_known`, `canary_supported`, `inventory_fresh` |
| Blocker | `runtime_capability_unknown`, `config_policy_missing`, `validation_probe_missing`, `rollback_undefined`, `algorithm_classification_unknown` |
| Phase | `hybrid_first`, `replace_now`, `monitor` |

`agility.score`는 enabler 점수에서 blocker 페널티를 차감한 0~100 값이다. 이 값은 위험도 점수와 별개이며, "얼마나 위험한가"가 아니라 "얼마나 안전하게 바꿀 수 있는가"를 나타낸다.
