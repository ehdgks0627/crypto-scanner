...
본 시스템의 영구 저장소는 PostgreSQL 16이며, ORM은 Django ORM을 사용한다. `Asset` row는 CBOM `components[]`의 원소와 1:1로 대응하고, `AssetDependency` row는 CBOM `dependencies[]`의 관계를 표현한다. CBOM JSON은 별도 원본으로 저장하지 않고 export 시 DB row에서 조립한다.
...
erDiagram
    ...
    CbomSnapshot ||--o{ AssetDependency : contains
    Asset ||--o{ AssetDependency : "from"
    Asset ||--o{ AssetDependency : "to"
...
| `algorithm` | CharField(128) | default `""` | CBOM `cryptoProperties.algorithm` 색인 |
| `algorithm_family` | CharField(64) | default `""` | 알고리즘 패밀리 색인 |
| `discovered_at` | DateTimeField | not null | |

**인덱스**:
- `(snapshot, asset_type)`
- `(snapshot, target)`
- `(bom_ref)` — Diff 시 빠른 매칭용
...
| `id` | BigAutoField | PK | |
| `snapshot` | ForeignKey(CbomSnapshot) | not null, on_delete=CASCADE | |
| `source_asset` | ForeignKey(Asset, related_name='dependency_edges') | not null, on_delete=CASCADE | CBOM dependency `ref` |
| `target_asset` | ForeignKey(Asset, related_name='depended_by_edges') | not null, on_delete=CASCADE | CBOM dependency `dependsOn[]` 항목 |
| `relation_type` | CharField(32) | default=`dependsOn` | export 시 CycloneDX 표준 `dependsOn`으로 평탄화 |
| `semantic` | CharField(64) | blank | `embeds_key`/`signed_by`/`uses_algorithm`/`uses_cert`/`hosts`/`protects_data`/`chains_to`/... |

**제약**: `(snapshot, source_asset, target_asset, relation_type)` unique.
