...
/snapshots                  식별 자산 목록 (전역 Snapshot 기준)
/snapshots/:id              Snapshot 자산 목록 직접 진입
...
```
┌─────────────────────────────────────────────────────────────────┐
│  [로고] PQC Risk Assessment     [Snapshot #56 ▼] [⏵ Job: 2] [🌗] │  Header (sticky)
├──────────┬──────────────────────────────────────────────────────┤
│          │                                                      │
...
| 영역 | 컴포넌트 | 설명 |
|---|---|---|
| Header | `<AppHeader />` | 로고, 전역 Snapshot 선택기, 활성 Job 카운터 (클릭 시 `/scans`), 다크모드 토글, 시스템 상태 인디케이터 (Worker/Redis 상태) |
...
**API**: `GET /api/dashboard/summary?snapshot_id={전역 선택 Snapshot}`.
...
- KpiCard 클릭 → 해당 필터링된 Asset Inventory로 이동
  - "자산수" 클릭 → `/snapshots`
  - "Critical" 클릭 → `/snapshots?tier=CRITICAL`
  - "양자취약" 클릭 → `/snapshots?quantum_vulnerable=true`
...
│ 신규 디스커버리                                          │
├─────────────────────────────────────────────────────────┤
│ CIDR: [172.31.240.0/24___________]                        │
│                                                         │
│ ☑ 기본 포트 사용 (테스트베드 표준)                       │
...
```
┌──────────────────────────────────────────────────────────┐
│ Discovery #4 — 172.31.240.0/24                             │
│ 상태: ⏳ RUNNING (4/256 호스트 스캔)                      │
│       ✅ COMPLETED — 33 endpoints 발견                    │
├──────────────────────────────────────────────────────────┤
│ ☑ Select All  [Promote 선택된 항목 →]                    │
│ ┌────────────────────────────────────────────────────┐  │
│ │ ☑  IP            Port   Proto   Suggested Host     │  │
│ │ ☑  172.31.240.10   443    TLS     web.testbed.local  │  │
│ │ ☑  172.31.240.10   22     ?       (none)             │  │
│ │ □  172.31.240.11   443    TLS     pqc-tls.testbed... │  │
│ │ ☑  172.31.240.13   8883   MQTT    mqtt.testbed.local │  │
│ │ ...                                                │  │
│ └────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────┘
```
...
### 9.4.10 Identified Assets (`/snapshots`, `/snapshots/:id`)

**목적**: CBOM 자산 탐색의 메인 페이지. 발표자료의 keycloak 예시와 유사.

**API**: `GET /api/snapshots`, `GET /api/snapshots/{id}`, `GET /api/snapshots/{id}/assets`.

**Snapshot 선택 정책**:
- Snapshot은 별도 목록 화면의 주인공이 아니라 조회 기준을 정하는 보조 기능이다.
- `/snapshots`는 헤더의 전역 Snapshot 선택값으로 바로 식별 자산 목록을 표시한다.
- `/snapshots/:id`는 공유 링크/직접 진입용이며 진입 시 헤더 전역 선택값도 해당 id로 맞춘다.
- 개별 메뉴 내부에는 Snapshot 선택 드롭다운을 두지 않는다.

**레이아웃**:

```
┌────────────────────────────────────────────────────────────────┐
│ 식별 자산 · Snapshot #56 (2026-04-25 10:05)  [📥 CBOM JSON]     │
│                                              [🔀 Diff]          │
│                                              [⚖ Risk]           │
│                                              [🚀 Migration]    │
├────────────────────────────────────────────────────────────────┤
...
**컴포넌트**:
- `<GlobalSnapshotSelector />` — Header의 전역 Snapshot 드롭다운 (`GET /api/snapshots`)
...
### 9.4.11 Asset Detail (`/snapshots/:id/assets/:aid`)
...
### 9.4.12 Snapshot Diff (`/snapshots/:id/diff`)
...
### 9.4.13 Risk Assessment (`/snapshots/:id/risk`)
...
### 9.4.14 Migration Plan (`/snapshots/:id/migration`)
...
독립 `/cbom` 화면은 두지 않는다. `Asset`이 곧 CBOM `components[]`이므로 조회 UX는 `/snapshots/:id`의 식별 자산 목록으로 통합하고, 표준 CycloneDX JSON은 Snapshot Detail의 `[CBOM JSON]` 액션에서 `GET /api/snapshots/{id}/export`로 다운로드한다.

### 9.4.16 Agents (`/agents`)
