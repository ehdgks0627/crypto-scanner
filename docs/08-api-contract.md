...
LLM 정성 분석 요청 (6.6, Mock 응답). 현재 응답은 DHS 6개 기준 중 Q1 `asset_value`, Q2 `protected_information`, Q3 `communication_scope`, Q4 `sharing_level`을 구조화해 포함한다.
...
LLM provider 응답은 자유 텍스트 안에 포함된 JSON 객체를 파싱하며, `summary`, `threat_scenarios`, `migration_recommendation`, `dhs_criteria.asset_value`, `dhs_criteria.protected_information`, `dhs_criteria.communication_scope`, `dhs_criteria.sharing_level`, `confidence` 필드를 구조화된 `QualitativeAssessment`로 정규화한다. `asset_value`는 외부 노출과 비즈니스 중요도를 기준으로, `protected_information`은 데이터 분류와 기밀성 요구를 기준으로, `communication_scope`는 내부 전용과 외부 양방향 노출 여부를 기준으로, `sharing_level`은 3rd-party·파트너·공개 클라이언트 연동 여부를 기준으로 `rating`, `score`, `rationale`, `signals`를 제공한다.
