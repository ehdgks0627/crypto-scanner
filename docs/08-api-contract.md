...
| | POST | `/api/discoveries` | CIDR/IP/domain 탐색 시작 |
  ...
  "items": [
    {
      ...
      "executor_type": "agent",
      "agent_id": "f1e2d3c4-0000-4000-8000-000000000002",
...
`executor_type`은 기본값 `central`이다. `agent`를 사용하면 `agent_id`로 지정한 Discovery Agent가 배치된 네트워크 위치에서 탐색한다. `include_default_ports`는 legacy CIDR 클라이언트 호환 필드이며, 신규 UI는 서비스 선택 결과를 `ports`로 직접 보낸다.
  ...
  "qualitative": {
    ...
    "dhs_criteria": {
      ...
      "critical_infrastructure": {
        ...
        "rationale": "...",
        "signals": ["service_role:customer-api", "dependency_level:critical"]
      },
      "protection_duration": {
...
LLM 정성 분석 요청 (6.6). 기본값은 `mock-rulebook` 응답이며, `QUALITATIVE_LLM_PROVIDER=openai-compatible`과 모델/API 키 환경변수가 설정되면 실제 LLM provider를 호출한다. 현재 응답은 DHS 6개 기준 중 Q1 `asset_value`, Q2 `protected_information`, Q3 `communication_scope`, Q4 `sharing_level`, Q5 `critical_infrastructure`, Q6 `protection_duration`을 구조화해 포함한다.
...
LLM provider 응답은 자유 텍스트 안에 포함된 JSON 객체를 파싱하며, `summary`, `threat_scenarios`, `migration_recommendation`, `dhs_criteria.asset_value`, `dhs_criteria.protected_information`, `dhs_criteria.communication_scope`, `dhs_criteria.sharing_level`, `dhs_criteria.critical_infrastructure`, `dhs_criteria.protection_duration`, `confidence` 필드를 구조화된 `QualitativeAssessment`로 정규화한다. `asset_value`는 외부 노출과 비즈니스 중요도를 기준으로, `protected_information`은 데이터 분류와 기밀성 요구를 기준으로, `communication_scope`는 내부 전용과 외부 양방향 노출 여부를 기준으로, `sharing_level`은 3rd-party·파트너·공개 클라이언트 연동 여부를 기준으로, `critical_infrastructure`는 DB·인증·결제·KMS·게이트웨이 의존도를 기준으로, `protection_duration`은 보존 기간과 HNDL 노출 가능성을 기준으로 `rating`, `score`, `rationale`, `signals`를 제공한다.
정규화된 6개 기준은 `dhs_risk`로 다시 합산된다. `protection_duration`에 가장 큰 가중치(1.6)를 두고, 전체 가중 평균을 0~10 `score_10`과 `P1/P2/P3` 우선순위로 변환한다.
provider 응답 파싱 실패, 타임아웃, 호출 오류가 발생하면 자산 메타데이터와 컨텍스트 기반 휴리스틱 결과로 폴백한다. 이 경우 `provider`는 `mock-rulebook-fallback`으로 저장되고, `prompt_payload.llm_fallback`에 사용 여부와 실패 사유를 기록한다. 실제 provider 호출이 성공하면 `prompt_payload.llm_provider`에 provider, model, usage 메타데이터를 함께 저장한다.
정상 생성된 정성 분석은 `prompt_payload.llm_cache.key`에 prompt 입력 해시를 저장하고, `prompt_payload.llm_cache.provider`에 provider/model 식별자를 저장한다. 이후 같은 prompt 입력과 같은 provider 설정으로 다시 요청되면 provider 호출 없이 저장된 `QualitativeAssessment`를 반환한다.
...
가중치 변경 후 재계산. 자산에 정성 분석 결과가 있으면 DHS 6기준 가중 합산 결과도 `dhs_risk`로 함께 저장한다.
...
쿼리 파라미터: `active`, `agent_role` (`host`/`discovery`), `offset`, `limit`.
