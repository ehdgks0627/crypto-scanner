...
## 17.5 DHS 2021 PQC 준비 로드맵

`미국 국토안보부(DHS) — Preparing for Post-Quantum Cryptography (2021)`는 PQC 전환 준비 절차의 초기 공식 근거로 사용한다. 구조화된 근거는 `docs/kpi/dhs-pqc-roadmap-evidence.json`에 둔다.

확인한 범위는 다음과 같다.

| 항목 | 확인 내용 |
| --- | --- |
| 공식 페이지 | DHS `Post-Quantum Cryptography` |
| 공식 URL | `https://www.dhs.gov/quantum` |
| 메모 문서 | `Memorandum on Preparing for Post-Quantum Cryptography` |
| 메모 URL | `https://www.dhs.gov/publication/memorandum-preparing-post-quantum-cryptography` |
| 핵심 주장 | DHS와 NIST는 조직이 PQC 전환에 대비하기 위해 현재 암호 시스템, 보호 데이터, 전환 우선순위를 파악해야 한다고 설명함 |
| 로드맵 흐름 | 표준 동향 추적 → 중요 데이터 인벤토리 → 암호 시스템 인벤토리 → 내부 기준 업데이트 식별 → 공개키 사용 위치 식별 → 위험 기반 우선순위화 → 전환 계획 수립 |

발표에서는 다음 수준으로 제한한다.

> DHS와 NIST의 2021 로드맵은 PQC 전환을 바로 제품 교체로 시작하지 않고, 먼저 보호해야 할 데이터와 현재 사용 중인 암호 기술을 목록화한 뒤 위험도에 따라 전환 우선순위를 정하라고 설명합니다.

주의할 점은 이 문서를 “모든 조직에 특정 기한까지 PQC 전환을 강제하는 법적 명령”으로 설명하면 안 된다는 것이다. DHS 내부 메모의 제출 요구는 DHS 구성기관을 대상으로 한 준비 지침이며, 공개 로드맵은 조직이 지금 수행할 수 있는 준비 절차를 제시한 guidance로 사용한다.
