...
export function PerformanceEvaluationView({ snapshotId }: { snapshotId: number }) {
  ...
  return (
    <Section>
      <PageHeader
        title={`Snapshot #${snapshotId} Performance`}
        description="PQC 전환 전후 성능평가 run과 자산별 측정 결과를 조회합니다."
      />
      <Card>
        <CardContent>
          <div className="toolbar">
            <div className="toolbar__filters">
              <Field className="field-inline">
                ...
                <Select aria-label="Performance profile filter" value={profile} onChange={(event) => setFilter("profile", event.target.value)}>
              ...
              <Field className="field-inline">
                ...
                <Select
                  aria-label="Performance run selector"
                  disabled={runItems.length === 0}
                  value={activeRunId ?? ""}
                  onChange={(event) => setFilter("run", event.target.value)}
                >
                  {runItems.length === 0 ? <option value="">run 없음</option> : null}
                  {runItems.map((run) => (
                    <option key={run.id} value={run.id}>
                      #{run.id} · {run.trigger} · {formatDateTime(run.created_at)}
                    </option>
                  ))}
                </Select>
              </Field>
      ...
      {runs.data && runItems.length === 0 ? (
        <EmptyState title="성능평가 run이 없습니다" description="마이그레이션 후 agent가 성능 측정 결과를 업로드하면 이곳에 표시됩니다." />
      ) : null}
  ...
  return (
    <div className="section-stack">
      <div className="content-grid content-grid--4">
        <MetricCard label="Overall" value={summary.overall_status} />
        <MetricCard label="Results" value={formatNumber(summary.total_results)} />
        <MetricCard label="Warn/Fail" value={formatNumber(warnFailCount)} />
        <MetricCard label="Baseline" value={run.baseline_snapshot_id ? `#${run.baseline_snapshot_id}` : "-"} />
      </div>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <dl className="detail-list">
            <div><dt>Status</dt><dd><PerformanceStatusBadge status={run.status} /></dd></div>
      ...
      <Card>
        ...
        <CardContent>
          <DataTable
            ...
            columns={[
              { key: "asset", header: "Asset", render: (item) => <span className="mono">{item.bom_ref}</span> },
              { key: "target", header: "Target", render: (item) => item.target_label ?? "-" },
              { key: "status", header: "Status", render: (item) => <PerformanceStatusBadge status={item.status} /> },
              { key: "compatibility", header: "Compatibility", render: (item) => <PerformanceStatusBadge status={item.compatibility_status} /> },
              { key: "algorithm", header: "Negotiated", render: (item) => item.negotiated_algorithm || "-" },
              { key: "handshake", header: "Handshake p95", align: "right", render: (item) => formatMs(metricP95(item, "handshake_ms")) },
              { key: "ttfb", header: "TTFB p95", align: "right", render: (item) => formatMs(metricP95(item, "ttfb_ms")) },
              { key: "failure", header: "Failure", align: "right", render: (item) => formatPercent(numberMetric(item, "failure_rate")) },
              { key: "delta", header: "Handshake Δ", align: "right", render: (item) => formatSignedPercent(item.deltas.handshake_p95_percent) },
              { key: "recommendation", header: "Recommendation", render: (item) => item.recommendation }
            ]}
          />
        </CardContent>
      </Card>
