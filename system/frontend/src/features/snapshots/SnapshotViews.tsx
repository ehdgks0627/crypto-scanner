...
export function CbomExportView() {
  ...
  return (
    <Section>
      ...
      <Card>
        <CardHeader><CardTitle><FileJson size={16} /> CBOM JSON</CardTitle></CardHeader>
        <CardContent>
          {cbom.isLoading ? <LoadingState /> : cbom.data ? <JsonPreview value={cbom.data} /> : <EmptyState title="스냅샷을 선택하세요" />}
        </CardContent>
      </Card>
    </Section>
  );
